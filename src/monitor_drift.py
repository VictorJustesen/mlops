...
def check_data_drift(
    trained_data, new_data, model_path, data_path="data/grouped", output_dir="data/drift"
):
    """
    ...
    trained_train_files = [
        os.path.join(data_path, f"{region}_train.csv") for region in trained_data
    ]
...
@app.command()
def cli(
    trained_regions: str = typer.Option(
        ..., help="Comma-separated list of already trained regions"
    ),
...
def pairwise_drift_inspector(
    ...
    target_seq = SequenceDataset(
        [target_file], window_size=window_size, input_features=input_features
    )
    target_df = pd.DataFrame(
        [
            x.flatten().tolist() + [float(y)]
            for x, y in zip(target_seq.sequences, target_seq.targets)
        ],
    ...
    all_files = [
        ...
        if f.endswith("_train.csv") and not f.startswith(target_region)
    ]
    for other_file in all_files:
        ...
        other_seq = SequenceDataset(
            [other_path], window_size=window_size, input_features=input_features
        )
        other_df = pd.DataFrame(
            [
                x.flatten().tolist() + [float(y)]
                for x, y in zip(other_seq.sequences, other_seq.targets)
            ],
            columns=[f"f{i}" for i in range(window_size * len(input_features))] + ["target"],
        )
        ...
        report_path = os.path.join(
            output_dir, f"Pairwise_drift_{target_region}_vs_{other_region}_{timestamp}.html"
        )
        report.save_html(report_path)
...
@app.command()
def pairwise_cli(
    target_region: str = typer.Option(
        ..., help="Region to use as reference for pairwise drift check"
    ),
    data_path: str = typer.Option("data/grouped", help="Path to region CSV files"),
    ...
    input_features: str = typer.Option(
        "Price,Load,Production", help="Comma-separated input features"
    ),
):
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        regions = [
            f.split("_train.csv")[0] for f in os.listdir(data_path) if f.endswith("_train.csv")
        ]
